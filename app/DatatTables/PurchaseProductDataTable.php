...
class PurchaseProductDataTable extends DataTable
{
    ...
    public function ajax()
    {
        return datatables()
            ...
            ->addColumn('action', function ($data) {
                ...
                $actionBtn = '<a href="/purchases/'.Encryption::encodeId($data->id).'/edit/" class="btn btn-xs btn-info" title="Edit Purchase Product"><i class="fa fa-edit"></i> Edit</a> ';
                $actionBtn .= '<a href="/purchases/'.Encryption::encodeId($data->id).'/delete/" class="btn btn-xs btn-danger action-delete" title="Delete Purchase Product"><i class="fa fa-trash"></i> Delete</a>';

                return $actionBtn;
            })
            ->editColumn('date',function($data){
                return ($data->date) ? Carbon::parse($data->date)->format('d F, Y ') : 'N/A';
            })
...
//            ->removeColumn('id')
    ...
    {
        $account = Purchase::getPurchaseProductList();
        $account->select([
            'purchases.*',
...
//            ->minifiedAjax('', null, request()->only(['from', 'to', 'team', 'user', 'category', 'status']))
    ...
    protected function getColumns()
    {
        return [
            ...
            'product'        => ['data' => 'product', 'name' => 'products.name', 'orderable' => true, 'searchable' => true],
            'category'       => ['data' => 'product_category', 'name' => 'product_categories.name', 'orderable' => true, 'searchable' => true],
            'product_code'   => ['data' => 'product_code', 'name' => 'product.product_code', 'orderable' => true, 'searchable' => true],
